...
class TeacherChannelScreen extends StatelessWidget {
  ...
  const TeacherChannelScreen({super.key, this.initialClassroomId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => getIt<TeacherChannelCubit>(),
      child: _TeacherChannelContent(initialClassroomId: initialClassroomId),
    );
  }
}

class _TeacherChannelContent extends StatefulWidget {
  final String? initialClassroomId;

  const _TeacherChannelContent({this.initialClassroomId});

  @override
  State<_TeacherChannelContent> createState() => _TeacherChannelContentState();
}

class _TeacherChannelContentState extends State<_TeacherChannelContent> {
  final ClassroomCubit _classroomsCubit = getIt<ClassroomCubit>();

  List<ClassroomModel> _classrooms = const [];
  ...
  Future<void> _loadClassrooms() async {
    ...
    await _classroomsCubit.getClassrooms();
    if (!mounted) return;
    ...
    final channelCubit = context.read<TeacherChannelCubit>();
  ...
  Future<void> _refreshQuestions() {
    final channelCubit = context.read<TeacherChannelCubit>();
    return channelCubit.getQuestions(
      classroomId: _activeClassroomId,
      sortBy: channelCubit.state.sortBy,
      filterBy: channelCubit.state.filterBy,
    );
  }
  ...
  Widget _buildBody() {
    ...
    return BlocBuilder<TeacherChannelCubit, TeacherChannelState>(
  ...
  Widget _buildFeed(BuildContext context, TeacherChannelState state) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "أنشئ فصلاً دراسياً لتتمكن من متابعة قناة أسئلته وإدارة نقاشات طلابك.",
              textAlign: TextAlign.center,
              style: AppTextStyles.body.copyWith(
                color: AppColors.textSecondary,
              ),
